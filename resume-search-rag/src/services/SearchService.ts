import ResumeRepository, { BM25Result } from "../repositories/ResumeRepository";
...
export default class SearchService {
  ...
  async bm25Search(query: string, topK = 20, filters: Record<string, any> = {}) {
    const start = Date.now();
    const results: BM25Result[] = await this.repo.bm25Search(query, topK, filters);
    const latencyMs = Date.now() - start;
    return { results, meta: { topK, latencyMs } };
  }

  async vectorSearch(query: string, topK = 20, filters: Record<string, any> = {}) {
    const start = Date.now();
    ...
    try {
      ...
      const embedding = await embeddingPromise as number[];
      ...
      try {
        ...
        const latencyMs = Date.now() - start;
        const diagnostics: any = { message: err?.message || String(err) };
        if (err?.index) diagnostics.index = err.index;
        if (err?.path) diagnostics.path = err.path;
        if (err?.expectedDimensions) diagnostics.expectedDimensions = err.expectedDimensions;
        if (err?.raw) diagnostics.raw = err.raw;
  ...
  async hybridSearch(query: string, topK = 20, filters: Record<string, any> = {}) {
    const start = Date.now();
    ...
    const result: any = { bm25: bm25Results, vector: vectorResults, errors: {} };
    
    if (bm25Error) result.errors.bm25 = bm25Error;
    if (vectorError) result.errors.vector = vectorError;

    const latencyMs = Date.now() - start;
    return { results: result, meta: { topK, latencyMs } };
  }

  async endToEndSearch(query: string, options: { topK?: number; filters?: Record<string, any>; rerankTopK?: number; summarize?: boolean; summarizeTopK?: number; summarizeOptions?: any } = {}) {
    const start = Date.now();
    ...
    const errors: any = {};
    const bm25Results: any[] = bm25ResP.status === 'fulfilled' ? bm25ResP.value : [];
    if (bm25ResP.status === 'rejected') errors.bm25 = (bm25ResP as PromiseRejectedResult).reason?.message || String((bm25ResP as PromiseRejectedResult).reason);
    const vectorResults: any[] = vectorResP.status === 'fulfilled' ? vectorResP.value : [];
    if (vectorResP.status === 'rejected') errors.vector = (vectorResP as PromiseRejectedResult).reason?.message || String((vectorResP as PromiseRejectedResult).reason);

    // Merge and deduplicate candidates: preserve BM25 order first, then vector
    const seen = new Set<string>();
    const merged: { resumeId: string; snippet: string }[] = [];

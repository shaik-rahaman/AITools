...
// POST /v1/search/bm25
router.post("/bm25", async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    logger.info({ msg: "search.bm25.request", requestId, queryLen: query.length, topK });
...
router.post("/vector", async (req: Request, res: Response, next: NextFunction) => {
  try {
    const requestId = (req as any).requestId || null;
    const { query, topK = 20, filters = {} } = req.body || {};
    ...
    logger.info({ msg: "search.vector.request", requestId, queryLen: query.length, topK });
...
router.post("/hybrid", async (req: Request, res: Response, next: NextFunction) => {
  try {
    const requestId = (req as any).requestId || null;
    const { query, topK = 20, filters = {} } = req.body || {};
    ...
    logger.info({ msg: "search.hybrid.request", requestId, queryLen: query.length, topK });
...
router.post("/rerank", async (req: Request, res: Response, next: NextFunction) => {
  try {
    const requestId = (req as any).requestId || null;
    ...
    logger.info({ msg: "search.rerank.request", requestId, queryLen: query.length, candidateCount: candidates.length, topK });
...
router.post("/summarize", async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // - { resumeId, snippet }
    // - { id, text }
    let normalizedCandidate: any = null;
    if (candidate && typeof candidate === 'object') {
      ...
      if (resumeId && snippet) normalizedCandidate = { resumeId, snippet };
    }
...
router.post("/", async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    logger.info({ msg: "search.endtoend.request", requestId, queryLen: query.length, topK, rerankTopK, summarize });
    ...
    let candidates = base.results as Array<{ resumeId: string; snippet: string }>;
    const meta: any = { ...base.meta };
    ...
    if ((!llm) && (rerankTopK > 0 || summarize)) {
      try {
        ...
        logger.warn({ msg: 'llm.init.failed', err: e?.message });
        llm = null;
      }
    }
    ...
    if (llm && candidates.length && rerankTopK > 0) {
      try {
        ...
        for (const s of sorted) {
          const id = s.resumeId || s.id || (s as any)._id;
          if (!id) continue;
        ...
        for (const c of finalOrdered) {
          if (!seen.has(c.resumeId)) ordered.push(c);
        }

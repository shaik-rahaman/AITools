...
class LLMService {
    constructor() {
        ...
        const explicitLLM = process.env.LLM_API_URL || env_1.config.llmApiUrl;
        ...
        if (!explicitLLM && groqKey) {
            ...
            this.apiUrl = explicitLLM || env_1.config.llmApiUrl;
            this.apiKey = env_1.config.llmApiKey || process.env.LLM_API_KEY || process.env.LLM_KEY;
    ...
    async rerankCandidates(query, candidates, topK) {
        // Detect Groq usage
        const parsed = (() => { try {
        ...
        if (useGroq) {
            ...
            const body = { model: env_1.config.llmModel || 'meta-llama/llama-4-scout-17b-16e-instruct', prompt };
            ...
            if (parsedCfg) {
                ...
                if (sanitizedBase && sanitizedBase !== rawBase)
                    baseCandidates.push(sanitizedBase);
                baseCandidates.push(parsedCfg.origin);
                const originPlusPath = parsedCfg.origin + (parsedCfg.pathname || '').replace(/\/+$/, '');
                if (originPlusPath && originPlusPath !== sanitizedBase && originPlusPath !== rawBase)
                    baseCandidates.push(originPlusPath);
            }
            if (useGroq)
            ...
            for (const base of Array.from(new Set(baseCandidates))) {
                for (const ep of endpoints) {
                    ...
                    try {
                        const resp = await axios_1.default.post(url, { model: body.model, input: prompt }, { headers: { 'Authorization': `Bearer ${this.apiKey}`, 'Content-Type': 'application/json' }, timeout: 20000 });
                        // parse outputs
                                ...
                                try {
                                    ...
                                    if (Array.isArray(parsedJson)) {
                                        ...
                                        for (const id of parsedJson) {
                                            const found = candidates.find(c => c.resumeId === id || c.id === id);
                                            if (found)
                                                ordered.push(found);
                                        }
                                        return ordered;
                        ...
                        if (err.response && err.response.status === 404)
                            continue;
                        const e = new Error('LLM upstream error');
        ...
        try {
            const response = await axios_1.default.post(`${this.apiUrl.replace(/\/+$/, '')}/rerank`, { query, candidates, topK }, { headers: { 'Authorization': `Bearer ${this.apiKey}` }, timeout: 20000 });
            if (Array.isArray(response.data?.sortedCandidates))
                return response.data.sortedCandidates;
            if (Array.isArray(response.data?.results))
                return response.data.results;
            if (Array.isArray(response.data?.data?.sortedCandidates))
                return response.data.data.sortedCandidates;
    ...
    async summarizeCandidateFit(query, candidate, options) {
        ...
        try {
            ...
            if (useGroq) {
                ...
                const body = {
                    model: env_1.config.llmModel || 'meta-llama/llama-4-scout-17b-16e-instruct',
                ...
                for (const base of Array.from(new Set(baseCandidates))) {
                    for (const ep of endpoints) {
                        ...
                        attemptedUrls.push(url);
                        logging_1.logger.info({ msg: 'llm.call.attempt', url, useGroq });
                        try {
                            ...
                            if (Array.isArray(resp.data?.outputs)) {
                                ...
                                if (Array.isArray(out?.content) && typeof out.content[0]?.text === 'string')
            ...
            const response = await axios_1.default.post(`${this.apiUrl.replace(/\/+$/, '')}/summarize`, {
                query,
                candidate,
                style: options.style,
            ...
            logging_1.logger.error({ msg: 'llm.summarize.error', error: err?.message || err, upstream: err?.response?.data || null });
            try {
    ...
    localSummarizeFallback(query, candidate, options) {
        ...
        if (!text)
            return 'No candidate text available for summarization.';
    ...
    async extractMetadata(rawText) {
        const response = await axios_1.default.post(`${this.apiUrl}/extract-metadata`, {
            text: rawText
        }, {
            headers: {
                'Authorization': `Bearer ${this.apiKey}`
